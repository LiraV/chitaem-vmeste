...
// Вход включается двумя переменными сборки. Проверяем их напрямую, чтобы не
// тащить SDK в основной бандл, когда аккаунты не настроены.
const AUTH_ON = Boolean(
...
function AuthScreen({ t, lang, auth, onLangCycle }) {
  const [mode, setMode] = useState("in");           // "in" | "up"
  const [email, setEmail] = useState("");
  const [pass, setPass] = useState("");
  const [busy, setBusy] = useState(false);
  const [note, setNote] = useState(null);
  const ready = Boolean(auth); // клиент ещё грузится — не даём отправить форму           // {kind:"err"|"ok", text}

  const field = { width: "100%", background: T.paperMsg, color: T.ink, ...px(), padding: "11px 12px",
    fontSize: 17, fontFamily: "'Neucha', cursive", outline: "none" };
  const label = { display: "block", marginTop: 14, marginBottom: 5, fontSize: 8.5, color: T.gold };

  async function submit(e) {
    e.preventDefault();
    if (busy) return;
    const mail = email.trim();
    // Проверяем до сети: так ошибка приходит мгновенно и на языке интерфейса.
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(mail)) return setNote({ kind: "err", text: t("authBadEmail") });
    if (pass.length < 6) return setNote({ kind: "err", text: t("authShortPass") });

    setBusy(true); setNote(null);
    const { data, error } = mode === "up"
      ? await auth.auth.signUp({ email: mail, password: pass })
      : await auth.auth.signInWithPassword({ email: mail, password: pass });
    setBusy(false);

    if (error) return setNote({ kind: "err", text: error.message || t("authFailed") });
    // Если в проекте включено подтверждение почты, сессии сразу не будет.
    if (mode === "up" && !data.session) setNote({ kind: "ok", text: t("authCheckMail") });
    // Иначе onAuthStateChange в App сам уведёт на библиотеку.
  }
  ...
  async function google() {
    ...
    if (error) { setBusy(false); setNote({ kind: "err", text: error.message }); }
    // При успехе браузер уходит на Google — возвращать busy не нужно.
  }
  ...
  return (
    <div className="view" style={{ position: "relative", zIndex: 1, minHeight: "100vh", display: "flex",
      ...
      <div className="hand" style={{ color: T.muted, fontSize: 17, marginTop: 6, textAlign: "center" }}>
        {t("authSubtitle")}
      </div>

      {/* noValidate: иначе браузер перехватывает неверную почту своей всплывашкой
          на своём языке, и получается разнобой с остальными ошибками. */}
      <form onSubmit={submit} noValidate style={{ width: "100%", maxWidth: 340, marginTop: 20 }}>
        <label className="pxfont" style={label} htmlFor="auth-email">{t("authEmail")}</label>
        <input id="auth-email" type="email" autoComplete="email" value={email}
          onChange={(e) => setEmail(e.target.value)} style={field} />

        <label className="pxfont" style={label} htmlFor="auth-pass">
          {t("authPass")} <span style={{ opacity: 0.65 }}>· {t("authPassHint")}</span>
        </label>
        <input id="auth-pass" type="password" value={pass}
          autoComplete={mode === "up" ? "new-password" : "current-password"}
          onChange={(e) => setPass(e.target.value)} style={field} />

        {note && (
          <div className="hand" style={{ marginTop: 12, fontSize: 16, lineHeight: 1.45,
            color: note.kind === "err" ? T.red : T.green }}>{note.text}</div>
        )}

        <button type="submit" disabled={busy || !ready} className="pxfont"
          style={{ width: "100%", marginTop: 16, ...px(), background: busy ? T.btn : T.gold,
            color: busy ? T.muted : "#3a2a10", padding: "13px 12px", fontSize: 10,
            cursor: busy ? "default" : "pointer", boxShadow: `4px 4px 0 rgba(0,0,0,0.35)` }}>
          {busy ? `${t("authWorking")}…` : mode === "up" ? t("authSignUp") : t("authSignIn")}
        </button>
      </form>

      <div className="hand" style={{ color: T.muted, fontSize: 15, margin: "16px 0 10px" }}>{t("authOr")}</div>
      <button onClick={google} disabled={busy || !ready} className="pxfont"
        style={{ width: "100%", maxWidth: 340, ...px(), background: T.paperMsg, color: T.ink,
          padding: "12px", fontSize: 9, cursor: "pointer" }}>
        {t("authGoogle")}
      </button>

      <button onClick={() => { setMode((m) => (m === "in" ? "up" : "in")); setNote(null); }}
        className="hand" style={{ marginTop: 18, background: "transparent", border: "none",
          color: T.gold, fontSize: 17, cursor: "pointer", textDecoration: "underline" }}>
        {mode === "in" ? t("authToSignUp") : t("authToSignIn")}
      </button>
    </div>
  );
}
...
export default function App() {
  ...
  const [session, setSession] = useState(AUTH_ON ? undefined : null);
  const [auth, setAuth] = useState(null); // клиент Supabase, подгружается лениво
  useEffect(() => {
    if (!AUTH_ON) return;
    let alive = true;
    let unsubscribe = () => {};
    import("./supabase").then(({ supabase }) => {
      ...
      const { data: sub } = supabase.auth.onAuthStateChange((_e, next) => { if (alive) setSession(next); });
      unsubscribe = () => sub.subscription.unsubscribe();
    }).catch((e) => {
      console.error("Не удалось загрузить модуль входа:", e);
  ...
  const signedIn = !AUTH_ON || Boolean(session);
