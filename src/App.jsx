...
// Способы входа. Работает пока только Яндекс; остальные показываем
// неактивными, чтобы экран не выглядел глухой стеной с одной кнопкой и было
// видно, куда движемся.
const AUTH_PROVIDERS = [
  { id: "yandex",   label: "Яндекс",   mark: "Я",  color: "#fc3f1d", ready: true },
  { id: "telegram", label: "Telegram", mark: "✈",  color: "#2aabee", ready: false },
  { id: "vk",       label: "VK",       mark: "VK", color: "#0077ff", ready: false },
  { id: "google",   label: "Google",   mark: "G",  color: "#e8e4d8", ready: false },
];

function AuthScreen({ t, lang, charId, onLangCycle, problem }) {
  return (
    <div className="view" style={{ maxWidth: 600, margin: "0 auto", position: "relative", zIndex: 1,
      minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      {/* Полка и зверёк — подпись приложения. Без них экран входа выглядел
          чужим: вся тёплая часть оставалась за дверью. */}
      <Bookshelf />
      <ShelfBuddy charId={charId} />

      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center",
        justifyContent: "center", padding: "18px 22px 32px" }}>

        <img src={IMG.logo} alt="" className="logo"
          style={{ width: 104, filter: "drop-shadow(4px 4px 0 rgba(0,0,0,0.45))" }} />

        <h1 className="pxfont" style={{ fontSize: 15, color: T.gold, margin: "16px 0 0", textAlign: "center",
          lineHeight: 1.6, textShadow: `3px 3px 0 ${T.outline}` }}>{t("authTitle")}</h1>

        <p className="hand" style={{ color: T.white, opacity: 0.92, fontSize: 19, lineHeight: 1.35,
          textAlign: "center", maxWidth: 330, margin: "12px 0 0" }}>{t("authPitch")}</p>

        {/* Причина входа — отдельной строкой и мягче тоном: человек должен
            понимать, что он получает, а не что у него просят. */}
        <div className="hand" style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 14,
          color: T.muted, fontSize: 16, textAlign: "center", maxWidth: 320 }}>
          <span style={{ fontSize: 17 }}>🔖</span>
          <span>{t("authSync")}</span>
        </div>

        {problem && (
          <div className="hand" style={{ marginTop: 14, fontSize: 16, color: T.red, textAlign: "center", maxWidth: 320 }}>
            {t("authFailed")}
          </div>
        )}

        <div style={{ width: "100%", maxWidth: 320, marginTop: 22, display: "grid", gap: 9 }}>
          {AUTH_PROVIDERS.map((p) => {
            const inner = (
              <>
                <span className="pxfont" style={{ width: 30, height: 30, flexShrink: 0, display: "inline-flex",
                  alignItems: "center", justifyContent: "center", background: p.ready ? p.color : "#3f4d42",
                  color: p.id === "google" && p.ready ? "#2c3a2e" : "#fff", fontSize: p.mark.length > 1 ? 8 : 11,
                  ...px(T.outline), borderWidth: 3 }}>{p.mark}</span>
                <span className="pxfont" style={{ flex: 1, textAlign: "left", fontSize: 9, lineHeight: 1.5 }}>
                  {p.ready ? `${t("authWith")} ${p.label}` : p.label}
                </span>
                {!p.ready && (
                  <span className="pxfont" style={{ fontSize: 7, color: T.muted, flexShrink: 0 }}>{t("authSoon")}</span>
                )}
              </>
            );
            const box = {
              display: "flex", alignItems: "center", gap: 11, width: "100%",
              padding: "10px 12px", ...px(p.ready ? T.gold : T.outline),
              background: p.ready ? T.paperMsg : "rgba(255,255,255,0.03)",
              color: p.ready ? T.ink : T.muted,
              textDecoration: "none",
              boxShadow: p.ready ? `4px 4px 0 rgba(0,0,0,0.4)` : "none",
            };
            return p.ready ? (
              <a key={p.id} href={`${import.meta.env.BASE_URL}api/auth/start`} style={box}>{inner}</a>
            ) : (
              <div key={p.id} aria-disabled="true" style={{ ...box, opacity: 0.5, cursor: "default" }}>{inner}</div>
            );
          })}
        </div>

        <button onClick={onLangCycle} className="pxfont" title="Language"
          style={{ marginTop: 22, background: "transparent", border: "none", color: T.muted,
            fontSize: 8, letterSpacing: 1, cursor: "pointer", opacity: 0.8 }}>
          {lang.toUpperCase()} ⌄
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ background: `linear-gradient(${T.night}, ${T.nightDeep})`, minHeight: "100vh", color: T.white,
      ...
      {auth === undefined ? (
        ...
        <AuthScreen t={t} lang={settings.lang} charId={settings.charId}
