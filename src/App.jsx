...
function AuthScreen({ t, lang, onLangCycle, problem }) {
  return (
    <div className="view" style={{ position: "relative", zIndex: 1, minHeight: "100vh", display: "flex",
      ...
      <img src={IMG.logo} alt="" style={{ width: 88, filter: "drop-shadow(4px 4px 0 rgba(0,0,0,0.45))" }} />
      ...
      <div style={{ width: "100%", maxWidth: 320, marginTop: 22, display: "grid", gap: 10 }}>
        {AUTH_PROVIDERS.map((p) => {
          const inner = (
            <>
              {/* Плашка с меткой провайдера — вместо логотипов, которых у нас нет. */}
              <span className="pxfont" style={{ width: 30, height: 30, flexShrink: 0, display: "inline-flex",
                alignItems: "center", justifyContent: "center", background: p.ready ? p.color : "#4a5a4e",
          ...
          const box = {
            ...
            background: p.ready ? T.paperMsg : "rgba(255,255,255,0.04)",
            color: p.ready ? T.ink : T.muted,
            textDecoration: "none",
            boxShadow: p.ready ? `4px 4px 0 rgba(0,0,0,0.35)` : "none",
          };
          return p.ready ? (
            <a key={p.id} href={`${import.meta.env.BASE_URL}api/auth/start`} style={box}>{inner}</a>
          ) : (
            <div key={p.id} aria-disabled="true" style={{ ...box, opacity: 0.55, cursor: "default" }}>{inner}</div>
          );
        })}
      </div>
    </div>
