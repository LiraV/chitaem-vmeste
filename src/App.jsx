...
function Reader({ t, chat, onBack, onUpdate, addMinutes }) {
  const [file, setFile] = useState(undefined); // undefined — ещё грузим
  const [err, setErr] = useState("");
  const [ch, setCh] = useState(chat.readerPos?.ch || 0);
  const [font, setFont] = useState(chat.readerFont || 19);
  const [toc, setToc] = useState(false);
  const [parsing, setParsing] = useState(false);
  const restoreTo = useRef(chat.readerPos?.y || 0);
  ...
  // Позиция и закладка сохраняются при смене главы, а не на каждый пиксель
  // прокрутки: запись в полку идёт через localStorage и стоит дорого.
  useEffect(() => {
    if (!chapters.length) return;
    const y = window.scrollY;
    const id = setTimeout(() => {
      onUpdate({ ...chat, progress: chapterLabel(ch), total: String(chapters.length),
        readerPos: { ch, y }, readerFont: font });
    }, 400);
  ...
  return (
    <div className="view" style={{ maxWidth: 600, margin: "0 auto", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ padding: "18px 16px 40px" }}>
        ...
        <label style={label}>{t("attachBook")} <span style={{ color: T.muted }}>{t("aboutOpt")}</span></label>
        <input ref={fileInput} type="file" accept=".epub,.fb2,.txt,.zip,.fbz" onChange={attachFile} style={{ display: "none" }} />
        <button onClick={() => fileInput.current?.click()} disabled={parsing} className="pxfont"
