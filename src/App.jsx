...
import { parseBookFile, ParseError, MAX_FILE_BYTES } from "./reader/parse";
import { getBookFile, putBookFile, deleteBookFile } from "./reader/store";
...
const THEMES = {
  ...
  library: {
    // «Библиотека» — палитра из пака ассетов (palette.json): глубокий изумруд
    // и тёплое золото. Значения взяты у дизайнера, а не подобраны на глаз.
    night: "#08241b", nightDeep: "#03140f", wood: "#7a5230", woodLight: "#9a6b3c", outline: "#514326",
    panelSolid: "#1c4732", gold: "#f4b949", green: "#67c27c", red: "#e0705f",
    paperMsg: "#f2ece0", ink: "#22301c", white: "#d9d0af", muted: "#8e7945",
    btn: "#0d2c20", cardBg: "rgba(8,36,27,0.92)", track: "#0b2418",
  },
  parchment: {
...
// Русскому числительному нужны три формы («1 глава», «2 главы», «5 глав»);
// остальным языкам приложения хватает двух, а японскому и китайскому — одной,
// записанной дважды.
...
async function findCover(title, author) {
  // Автор в запросе отсеивает одноимённые книги: без него «Мы» или «Оно»
  // приносили обложку первой попавшейся записи с таким словом.
  const by = author ? `&author=${encodeURIComponent(author)}` : "";
  const url = `${COVER_SEARCH}?title=${encodeURIComponent(title)}${by}&limit=8&fields=cover_i`;
...
async function searchBookInfo(title, author, lang) {
  const response = await fetch(API_ENDPOINT, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      max_tokens: 1000,
      messages: [{ role: "user", content: `О чём книга «${title}»${author ? ` автора ${author}` : ""}? Верни ТОЛЬКО краткое описание сюжета и сеттинга в 3–6 предложениях, без спойлеров второй половины книги, без оценок и предисловий.

Отвечай только если ТОЧНО знаешь именно эту книгу. Выдумывать описание запрещено: если книга тебе не знакома или ты не уверен, что речь о ней, ответь одним словом NOT_FOUND.` }],
      }),
  });
  const data = await response.json();
...
// Кэш подсказок: один и тот же запрос (и его повтор при дописывании и
// стирании) не должен второй раз стоить денег. Ключ — язык и нормализованный
// текст; размер ограничен, чтобы за долгую сессию карта не разрасталась.
const suggestCache = new Map();
const SUGGEST_CACHE_MAX = 80;

async function modelMatches(q, lang) {
  const cacheKey = `${lang}|${normTitle(q)}`;
...
BOOK: <название> | <автор> | <год первого издания>` }],
  ...
  const parsed = /NOTHING/i.test(raw) ? [] : raw.split("\n")
...
export async function searchBooks(q, lang, onUpdate) {
  ...
  const own = poolMatches(query, lang);
  add(own);
  // Пул дал полную горсть — модель не нужна: почти наверняка читатель ищет
  // одну из этих книг, а запрос к модели стоит денег.
  if (own.length < 5) {
...
// Дата ЛОКАЛЬНАЯ, а не UTC: по Гринвичу чтение в час ночи по Москве падало
// на «вчера», и день подряд то не засчитывался, то удваивался.
...
// Позиция чтения (глава, прокрутка, шрифт) живёт в собственном ключе, а не в
// записи книги: чат при каждом сохранении пишет свой снимок книги целиком, и
// когда читалка закрывалась, его снимок — снятый до выхода — перетирал только
// что сохранённую прокрутку. Отдельный ключ чату не виден, гонки нет.
const posKey = (id) => `chitaem-vmeste:reader-pos-${id}`;
function loadReaderPos(id, fallback) {
  try { return JSON.parse(localStorage.getItem(posKey(id))) || fallback; } catch { return fallback; }
}
function saveReaderPos(id, pos) {
  try { localStorage.setItem(posKey(id), JSON.stringify(pos)); } catch {}
}
function dropReaderPos(id) {
  try { localStorage.removeItem(posKey(id)); } catch {}
}

// ——— Чтение книги ———
// Файл читателя лежит в IndexedDB и разбирается здесь же, на устройстве.
// Главная выгода не в самом чтении, а в закладке: пока читатель листал книгу
// в чужой читалке, место в тексте приходилось вводить руками, и спойлер-защита
// держалась на честном слове. Отсюда она проставляется сама, точно по главе.
function Reader({ t, chat, onBack, onUpdate, addMinutes, onFinish }) {
  const [file, setFile] = useState(undefined); // undefined — ещё грузим
  const [err, setErr] = useState("");
  // chat.readerPos — наследие ранних версий, когда позиция лежала в книге.
  const pos0 = loadReaderPos(chat.id, chat.readerPos || {});
  const [ch, setCh] = useState(pos0.ch || 0);
  const [font, setFont] = useState(pos0.font || chat.readerFont || 19);
  const [toc, setToc] = useState(false);
  const [parsing, setParsing] = useState(false);
  const [sureDelete, setSureDelete] = useState(false);
  const restoreTo = useRef(pos0.y || 0);
  const fileInput = useRef(null);
  // Текущая прокрутка — в ref по событию scroll, в хранилище — с паузой:
  // писать на каждый пиксель незачем, а полсекунды после остановки — дёшево.
  const lastY = useRef(pos0.y || 0);
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      clearTimeout(timer);
      timer = setTimeout(() => saveReaderPos(chat.id, { ch: chRef.current, y: lastY.current, font: fontRef.current }), 500);
    };
    window.addEventListener("scroll", onScroll, { passive: true });
    return () => {
      window.removeEventListener("scroll", onScroll);
      clearTimeout(timer);
      saveReaderPos(chat.id, { ch: chRef.current, y: lastY.current, font: fontRef.current });
    };
  }, []);

  // Книгу, уже стоящую на полке, можно снабдить файлом отсюда: заводить её
  // заново ради этого было бы странно.
  async function attachFile(e) {
    ...
    try {
      const parsed = await parseBookFile(f);
      await putBookFile(chat.id, { name: f.name, size: f.size, book: parsed });
      setFile({ name: f.name, size: f.size, book: parsed });
      setCh(0);
    } catch {
      setErr(f.size > MAX_FILE_BYTES ? t("attachBig") : t("attachFail"));
    }
    setParsing(false);
  }
  ...
  useEffect(() => {
    let alive = true;
    getBookFile(chat.id)
      .then((f) => { if (alive) setFile(f || null); })
      .catch(() => { if (alive) { setFile(null); setErr(t("attachFail")); } });
    return () => { alive = false; };
  }, [chat.id]);

  // Время чтения. Считаем только когда вкладка на виду: телефон в кармане с
  // открытым экраном чтения не должен приписывать читателю часы.
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (++seconds >= 60) { seconds = 0; addMinutesRef.current(1); }
    }, 1000);
    return () => clearInterval(id);
  }, []);

  const chapters = file?.book?.chapters || [];
  const current = chapters[Math.min(ch, chapters.length - 1)];

  // Закладка = название главы, а доля прочитанного считается по словам:
  // главы бывают очень разной длины, и «12 из 40» врало бы.
  const wordsBefore = useRef([]);
  useEffect(() => {
    let sum = 0;
    wordsBefore.current = chapters.map((c) => {
      const at = sum;
      sum += c.paragraphs.reduce((n, p) => n + p.split(/\s+/).length, 0);
      return at;
    });
    wordsBefore.current.total = sum || 1;
  }, [file]);

  const chapterLabel = (i) => chapters[i]?.title || `${t("chapterLbl")} ${i + 1}`;
  const share = chapters.length
    ? Math.round(((wordsBefore.current[ch] || 0) / (wordsBefore.current.total || 1)) * 100)
    : 0;

  // Закладка и доля сохраняются при смене главы, точная прокрутка — ещё и
  // при выходе с экрана (ниже): на каждый пиксель писать в полку дорого.
  //
  // «Всего глав или страниц», вписанное читателем руками, не перетираем:
  // он мог мерить книгу страницами. Свою долю читалка кладёт отдельным
  // полем readerShare — по словам, а не по номерам глав: главы разной длины.
  const saveSpot = () => {
    if (!chapters.length) return;
    const before = wordsBefore.current;
    const share = Math.min(1, ((before[ch] || 0) +
      (chapters[ch]?.paragraphs.reduce((m2, p2) => m2 + p2.split(/\s+/).length, 0) || 0) *
        Math.min(1, lastY.current / Math.max(1, document.documentElement.scrollHeight - window.innerHeight))) /
      (before.total || 1));
    saveReaderPos(chat.id, { ch, y: lastY.current, font });
    onUpdate({ ...chat, progress: chapterLabel(ch), total: chat.total || String(chapters.length),
      readerShare: share });
  };
  const saveRef = useRef(saveSpot);
  saveRef.current = saveSpot;
  useEffect(() => {
    if (!chapters.length) return;
    const id = setTimeout(() => saveRef.current(), 400);
    return () => clearTimeout(id);
  }, [ch, font, file]);


  // Возврат на прежнее место: только один раз, при первом показе главы.
  useEffect(() => {
    if (!current || !restoreTo.current) return;
  ...
  function goTo(i) {
    setCh(Math.max(0, Math.min(chapters.length - 1, i)));
    setToc(false);
    lastY.current = 0;
    window.scrollTo({ top: 0, behavior: "auto" });
  }

  const barBtn = { ...px(), background: T.btn, color: T.gold, padding: "8px 11px", fontSize: 9,
    cursor: "pointer", fontFamily: "'Press Start 2P', monospace", lineHeight: 1.4 };

  if (file === undefined) {
    return (
      <div className="view" style={{ maxWidth: 640, margin: "0 auto", padding: "24px 16px", position: "relative", zIndex: 1 }}>
        <p className="hand" style={{ color: T.muted, fontSize: 19, textAlign: "center" }}>{t("readerOpening")}<span className="cursor" /></p>
      </div>
    );
  }
  ...
  if (!file) {
    return (
      <div className="view" style={{ maxWidth: 640, margin: "0 auto", padding: "18px 16px 40px", position: "relative", zIndex: 1 }}>
        <button onClick={onBack} className="pxfont" style={smallBtn}>{t("back")}</button>
        <p className="hand" style={{ color: T.muted, fontSize: 19, margin: "24px 0 14px" }}>{err || t("readerNoFile")}</p>
        <input ref={fileInput} type="file" accept=".epub,.fb2,.txt,.zip,.fbz" onChange={attachFile} style={{ display: "none" }} />
        <button onClick={() => fileInput.current?.click()} disabled={parsing} className="pxfont"
          style={{ ...pxButton(!parsing), padding: "13px 18px", fontSize: 10 }}>
          {parsing ? <span>{t("readerOpening")}<span className="cursor" /></span> : t("attachBtn")}
        </button>
        <p className="hand" style={{ color: T.muted, fontSize: 16, margin: "10px 0 0", lineHeight: 1.3 }}>{t("attachHint")}</p>
      </div>
    );
  }

  return (
    <div className="view" style={{ maxWidth: 700, margin: "0 auto", position: "relative", zIndex: 1 }}>
      <div style={{ position: "sticky", top: 0, zIndex: 5, background: T.nightDeep, borderBottom: `4px solid ${T.outline}`,
        ...
        <button onClick={onBack} className="pxfont" style={barBtn}>←</button>
        <button onClick={() => setToc(!toc)} className="pxfont" style={{ ...barBtn, flex: 1, textAlign: "left", overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>
          {chapterLabel(ch)}
        </button>
        <button onClick={() => setFont(Math.max(15, font - 2))} className="pxfont" style={barBtn}>A-</button>
        <button onClick={() => setFont(Math.min(31, font + 2))} className="pxfont" style={barBtn}>A+</button>
      </div>

      <div style={{ height: 8, background: T.track }}>
        <div style={{ height: "100%", width: `${share}%`, background: T.gold }} />
      </div>

      {toc && (
        <div style={{ ...px(), background: T.panelSolid, margin: "10px 12px", maxHeight: 340, overflowY: "auto" }}>
          <p className="pxfont" style={{ fontSize: 9, color: T.gold, padding: "10px 12px 6px", margin: 0 }}>{t("contents")}</p>
          {chapters.map((c, i) => (
            <button key={i} onClick={() => goTo(i)} className="hand"
              style={{ display: "block", width: "100%", textAlign: "left", background: "transparent", border: "none",
                borderTop: `2px solid ${T.outline}`, padding: "9px 12px", fontSize: 17, cursor: "pointer",
                color: i === ch ? T.gold : T.white }}>
              {i + 1}. {chapterLabel(i)}
            </button>
          ))}
        </div>
      ...
      <div style={{ background: T.paperMsg, color: T.ink, margin: "12px 10px 0", padding: "20px 18px 26px", ...px() }}>
        {current?.title && (
          <h2 className="pxfont" style={{ fontSize: 12, lineHeight: 1.7, margin: "0 0 18px", color: "#6b5433" }}>{current.title}</h2>
        )}
        {(current?.paragraphs || []).map((para, i) => (
          <p key={i} className="hand" style={{ fontSize: font, lineHeight: 1.5, margin: "0 0 14px", textIndent: "1.4em" }}>{para}</p>
        ))}
      </div>

      <div style={{ textAlign: "center", padding: "18px 0 0" }}>
        <button onClick={() => {
            if (!sureDelete) { setSureDelete(true); setTimeout(() => setSureDelete(false), 2500); return; }
            deleteBookFile(chat.id).catch(() => {});
            setFile(null); setSureDelete(false);
          }} className="pxfont"
          style={{ background: "transparent", border: "none", color: sureDelete ? T.red : T.muted, fontSize: 8, cursor: "pointer", textDecoration: "underline" }}>
          {sureDelete ? t("sure") : t("removeFile")}
        </button>
      </div>
      <div style={{ display: "flex", gap: 9, padding: "14px 10px 30px" }}>
        <button onClick={() => goTo(ch - 1)} disabled={ch === 0} className="pxfont"
          style={{ ...pxButton(ch > 0), flex: 1, padding: "13px 0", fontSize: 9.5 }}>{t("prevCh")}</button>
        {ch >= chapters.length - 1 && !chat.finished ? (
          <button onClick={() => { saveRef.current(); onFinish(); }} className="pxfont"
            style={{ ...px(T.gold), background: T.gold, color: T.outline, flex: 1, padding: "13px 0", fontSize: 9.5, cursor: "pointer" }}>{t("finishBtn")}</button>
        ) : (
          <button onClick={() => goTo(ch + 1)} disabled={ch >= chapters.length - 1} className="pxfont"
            style={{ ...pxButton(ch < chapters.length - 1), flex: 1, padding: "13px 0", fontSize: 9.5 }}>{t("nextCh")}</button>
        )}
      </div>
    </div>
  );
      ...
      <div style={{ padding: "22px 16px 40px" }}>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          {chats.map((c) => {
            ...
            return (
              <div key={c.id} style={{ display: "flex", alignItems: "stretch", gap: 8, minWidth: 0 }}>
                <button onClick={() => onOpen(c.id)}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {(() => {
                      // Читалка знает долю точно (по словам); ручной ввод — по числам из закладки.
                      const share = c.finished ? 1 : (c.readerShare || readShare(c.progress, c.total));
                    ...
                    <div className="hand" style={{ fontSize: 15, marginTop: 5, color: "#8a7355" }}>
                      {comp.glyph} {comp.name[lang] || comp.name.ru} · <span style={{ whiteSpace: "nowrap" }}>{t("msgs")}: {c.messages.filter((m) => !m.meta && !m.hidden).length}</span>
                    </div>
                  </div>
                </button>
...
function Setup({ t, lang, pro, onStart, onBack }) {
  ...
  async function attachFile(e) {
    ...
    e.target.value = "";
    if (!f) return;
    setParsing(true); setFileErr("");
    try {
      const parsed = await parseBookFile(f);
      setBookFile({ name: f.name, size: f.size, book: parsed });
      // Название и автор из файла экономят читателю два поля.
      if (!book.trim() && parsed.title) { justPicked.current = true; setBook(parsed.title); }
      if (!author && parsed.author) setAuthor(parsed.author);
      if (!total.trim()) setTotal(String(parsed.chapters.length));
    } catch {
      setFileErr(f.size > MAX_FILE_BYTES ? t("attachBig") : t("attachFail"));
    }
    setParsing(false);
  }
  ...
  async function findInfo() {
    ...
    setSearching(true);
    try { setBookInfo(await searchBookInfo(book.trim(), author.trim(), lang)); } catch { setBookInfo(""); }
    setSearching(false);
  }
  ...
  return (
    <div className="view" style={{ maxWidth: 600, margin: "0 auto", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ padding: "18px 16px 40px" }}>
        ...
        <label style={label}>{t("attachBook")} <span style={{ color: T.muted }}>{t("optLbl")}</span></label>
        <input ref={fileInput} type="file" accept=".epub,.fb2,.txt,.zip,.fbz" onChange={attachFile} style={{ display: "none" }} />
        <button onClick={() => fileInput.current?.click()} disabled={parsing} className="pxfont"
          style={{ ...pxButton(!parsing), padding: "11px 16px", fontSize: 10 }}>
          {parsing ? <span>{t("readerOpening")}<span className="cursor" /></span> : t("attachBtn")}
        </button>
        {bookFile && (
          <p className="hand" style={{ color: T.green, fontSize: 17, margin: "8px 0 0", lineHeight: 1.3 }}>
            ✓ {bookFile.name} · {bookFile.book.chapters.length} {plural(lang, bookFile.book.chapters.length, t("chapterPlural").split("|"))} · {t("attachedLbl")}
          </p>
        )}
        {fileErr && <p className="hand" style={{ color: T.red, fontSize: 17, margin: "8px 0 0" }}>{fileErr}</p>}
        <p className="hand" style={{ color: T.muted, fontSize: 16, margin: "6px 0 0", lineHeight: 1.3 }}>{t("attachHint")}</p>
        ...
        <button disabled={!ready}
          onClick={() => onStart({ book: book.trim(), author: author.trim(), bookFile, progress: progress.trim(), companionId: companion.id, bookInfo: bookInfo.trim(), goal: goal.trim(), total: total.trim() })}
...
// ——— Чат ———
function Chat({ t, lang, settings, chat, onUpdate, onBack, addXp, addMinutes, onFlag, onRead }) {
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 5, background: T.nightDeep, borderBottom: `4px solid ${T.outline}` }}>
        ...
        <div style={{ padding: "10px 14px 12px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10, background: T.cardBg, ...px(finished ? T.green : T.gold), borderLeft: `8px solid ${finished ? T.green : T.gold}`, padding: "9px 12px" }}>
            ...
            {!finished && (
              <button onClick={finishBook} title={stripIcon(t("finishBtn"))} className="pxfont"
                style={{ ...px(T.gold), background: "transparent", color: T.gold, padding: "6px 9px", fontSize: 10, cursor: "pointer", flexShrink: 0 }}>
                🏁
              </button>
            )}
          </div>
          ...
          {toolsOpen && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginTop: 10 }}>
              <button onClick={() => { setToolsOpen(false); onRead(); }} style={toolBtn(true)}>{t("readLbl")}</button>
      ...
      <div style={{ position: "sticky", bottom: 0, background: T.nightDeep, borderTop: `4px solid ${T.outline}`, padding: "12px 14px calc(14px + env(safe-area-inset-bottom, 0px))" }}>
        <div style={{ display: "flex", gap: 8 }}>
          <textarea value={input} onChange={(e) => setInput(e.target.value)}
            ...
            placeholder={t("inputPh2")} rows={1} style={{ ...inputStyle, resize: "none", fontSize: 18, lineHeight: 1.3, maxHeight: 112, overflowY: "auto" }} />
          {SR && (
...
// Ассеты экрана входа лежат файлами; BASE_URL учитывает префикс пути.
const la = (name) => `${import.meta.env.BASE_URL}art/login/${name}.webp`;

const AUTH_PROVIDERS = [
  { id: "yandex",   label: "Яндекс",   logo: "yandex_logo",   ready: true },
  { id: "telegram", label: "Telegram", logo: "telegram_logo", ready: false },
  { id: "vk",       label: "VK",       logo: "vk_logo",       ready: false },
  { id: "google",   label: "Google",   logo: "google_logo",   ready: false },
];

function AuthScreen({ t, lang, onLangCycle, onGuest, problem }) {
  const features = [
    ["feature_chat_icon", t("authF1")],
    ["feature_shelf_icon", t("authF2")],
    ["feature_ai_icon", t("authF3")],
  ];

  return (
    <div className="view" style={{ maxWidth: 460, margin: "0 auto", position: "relative", zIndex: 1,
      minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center",
      padding: "26px 20px 34px" }}>

      {/* Полка целиком одной картинкой: растение, книги, спящий кот, свеча. */}
      <img src={la("top_shelf_full")} alt="" style={{ width: "100%", maxWidth: 320, height: "auto" }} />
      ...
      <div className="pxfont" style={{ fontSize: 9, color: T.white, opacity: 0.85, marginTop: 10,
        letterSpacing: 0.5 }}>{t("homeHint")}</div>

      <p className="hand" style={{ color: T.white, fontSize: 19, lineHeight: 1.4, textAlign: "center",
        maxWidth: 340, margin: "20px 0 0" }}>{t("authPitch")}</p>

      {/* Что человек получает — до того, как у него что-то просят. */}
      <div style={{ width: "100%", maxWidth: 360, marginTop: 20, background: T.cardBg,
        border: `2px solid ${T.outline}`, borderRadius: 14, padding: "6px 16px" }}>
      ...
      {problem && (
        <div className="hand" style={{ marginTop: 16, fontSize: 16, color: T.red, textAlign: "center", maxWidth: 340 }}>
          {t("authFailed")}
        </div>
      )}

      <div style={{ width: "100%", maxWidth: 360, marginTop: 18, display: "grid", gap: 9 }}>
        {AUTH_PROVIDERS.map((p) => {
          ...
          const box = {
            ...
            padding: p.ready ? "16px 18px" : "13px 18px", borderRadius: 14, textDecoration: "none",
          };
          return p.ready ? (
            <a key={p.id} href={`${import.meta.env.BASE_URL}api/auth/start`}
              style={{ ...box, background: `linear-gradient(180deg, ${T.gold}, #e0a63c)`, color: "#2a1e06",
                border: `2px solid ${T.gold}`, boxShadow: `0 0 18px rgba(244,185,73,0.35)` }}>{inner}</a>
          ) : (
            <div key={p.id} aria-disabled="true"
              style={{ ...box, background: "rgba(255,255,255,0.03)", color: T.muted,
                border: `2px solid ${T.outline}`, cursor: "default" }}>{inner}</div>
          );
        })}
      </div>
      ...
      <button onClick={onGuest} className="pxfont"
        style={{ width: "100%", maxWidth: 280, marginTop: 20, padding: "15px 12px", borderRadius: 14,
          background: "transparent", border: `2px solid ${T.outline}`, color: T.muted,
          fontSize: 10, cursor: "pointer" }}>
        {t("authGuest")}
      </button>

      <button onClick={onLangCycle} className="pxfont" title="Language"
        style={{ display: "inline-flex", alignItems: "center", gap: 8, marginTop: 22, padding: "9px 16px",
          borderRadius: 12, background: "transparent", border: `2px solid ${T.outline}`,
          color: T.white, fontSize: 10, cursor: "pointer" }}>
        {/* Глобус и шеврон из пака вырезаны неудачно — вместо знаков получаются
            полоски. Знаки надёжнее и выглядят как задумано. */}
...
export default function App() {
  ...
  function createChat({ bookFile, ...data }) {
    ...
    // Текст книги — мегабайты, и в localStorage рядом с полкой ему не место.
    if (bookFile) putBookFile(chat.id, bookFile).catch(() => {});
    fetchCoverFor(chat);
    setActiveId(chat.id);
    setView("chat");
  }
  ...
  function fetchCoverFor(chat) {
    ...
    findCover(chat.book, chat.author)
  ...
  // Дочитывание из читалки: то же, что кнопка 🏁 в чате, но чат сейчас не
  // смонтирован — помечаем книгу и кладём разделитель прямо в сохранённые
  // сообщения; поздравление собеседник скажет, когда чат откроется следом.
  function finishFromReader(chat) {
    if (!chat || chat.finished) return;
    addXp(50);
    persistChats((chatsRef.current || []).map((c) => c.id === chat.id
      ? { ...c, finished: true, readerShare: 1,
          messages: [...(c.messages || []), { id: Date.now(), role: "assistant", meta: "divider", content: t("finishDivider") }] }
      : c));
  }

  function deleteChat(id) {
    persistChats((chats || []).filter((c) => c.id !== id));
    deleteBookFile(id).catch(() => {});
    dropReaderPos(id);
  }
  ...
  const signedIn = !authOn || Boolean(session) || Boolean(settings.guest);
  ...
  return (
    <div style={{ background: `linear-gradient(${T.night}, ${T.nightDeep})`, minHeight: "100vh", color: T.white,
      ...
      {auth === undefined ? (
        ...
        <Reader key={`r${active.id}`} t={t} chat={active} onUpdate={updateChat} addMinutes={addMinutes}
          onBack={() => setView("chat")}
          onFinish={() => { finishFromReader(active); setView("chat"); }} />
      ) : view === "chat" && active ? (
        <Chat key={active.id} t={t} lang={settings.lang} settings={settings} chat={active} onUpdate={updateChat} onBack={() => { setActiveId(null); setView("library"); }} addXp={addXp} addMinutes={addMinutes} onFlag={raiseFlag} onRead={() => setView("reader")} />
      ) : (
