...
const COMPANIONS = [
  { id: "scholar", glyph: "📜",
    ...
    prompt: "Твой характер: интеллигентный литературовед. Говоришь глубоко, но живо: раскрываешь символы, исторический контекст, параллели с другими произведениями. Иногда цитируешь идею (не текст) и предлагаешь неожиданный угол зрения." },
  { id: "debater", glyph: "⚔️",
    ...
    prompt: "Твой характер: дерзкий, но обаятельный спорщик. Ты не соглашаешься с поверхностными оценками, защищаешь непопулярных персонажей, задаёшь неудобные вопросы и требуешь аргументов. Споришь азартно, но дружелюбно, без грубости." },
  { id: "friend", glyph: "☕",
    ...
    prompt: "Твой характер: тёплый близкий друг за чашкой чая. Делишься эмоциями и личными впечатлениями, сопереживаешь героям, спрашиваешь о чувствах собеседника. Просто, душевно, без академичности." },
  { id: "philosopher", glyph: "🕯️", pro: true,
    ...
    prompt: "Твой характер: философ-собеседник. Через события книги выводишь разговор к вечным вопросам: выбор, свобода, смысл, справедливость. Спокойный, глубокий, иногда отвечаешь вопросом на вопрос, но не занудствуешь." },
];
...
// ——— Хранилище ———
const DEFAULT_SETTINGS = { lang: "ru", charId: "cat", pro: false, xp: 0, spent: 0, tts: false, kids: false, owned: [], streak: { count: 0, last: "" }, minutes: 0, seenIntro: false };
...
const pxButton = (active) => ({ ...px(), background: active ? T.green : "#3a3a52", color: active ? T.outline : T.muted, boxShadow: active ? `inset -4px -4px 0 rgba(0,0,0,0.25), inset 4px 4px 0 rgba(255,255,255,0.25)` : "inset -4px -4px 0 rgba(0,0,0,0.3)", cursor: active ? "pointer" : "default", fontFamily: "'Press Start 2P', monospace" });
const smallBtn = { ...px(), background: "#3a3a52", color: T.muted, padding: "9px 12px", fontSize: 9, cursor: "pointer", fontFamily: "'Press Start 2P', monospace", lineHeight: 1.4 };
const toolBtn = (gold) => ({ ...px(gold ? T.gold : T.outline), background: "#3a3a52", color: gold ? T.gold : T.muted, padding: "10px 8px", fontSize: 8.5, cursor: "pointer", fontFamily: "'Press Start 2P', monospace", lineHeight: 1.5, textAlign: "center" });
...
function XpBar({ t, xp, streak }) {
  ...
  return (
    <div style={{ margin: "0 0 16px" }}>
      ...
      <div style={{ height: 14, ...px(), background: "#1c1a3a" }}>
...
// ——— Меню ———
function Menu({ t, settings, chats, onChange, onBack, onImport }) {
  ...
  const toggleRow = (label2, val, set) => (
    <button onClick={set} className="hand" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", background: "rgba(20,22,52,0.9)", ...px(val ? T.gold : T.outline), color: val ? T.gold : T.muted, padding: "11px 14px", fontSize: 18, cursor: "pointer", marginBottom: 8 }}>
  ...
  return (
    <div className="view" style={{ maxWidth: 600, margin: "0 auto", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ padding: "18px 16px 40px" }}>
        ...
        <h1 className="pxfont" style={{ fontSize: 18, color: T.gold, textAlign: "center", margin: "16px 0 4px", textShadow: `3px 3px 0 ${T.outline}` }}>{t("menuTitle")}</h1>

        <div style={label}>{t("langTitle")}</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {LANGS.map((l) => (
            <button key={l.id} onClick={() => onChange({ ...settings, lang: l.id })}
              style={{ ...px(settings.lang === l.id ? T.gold : T.outline), background: settings.lang === l.id ? T.panelSolid : "rgba(20,22,52,0.9)", boxShadow: settings.lang === l.id ? GLOW : "none", fontSize: 26, padding: "8px 12px", cursor: "pointer", lineHeight: 1 }}>
              {l.flag}
            </button>
          ))}
        </div>

        <div style={label}>{t("charTitle")} · {t("shopWallet")}: {wallet} XP</div>
        <p className="hand" style={{ color: T.muted, fontSize: 17, margin: "0 0 10px" }}>{t("charHint")}</p>
          ...
          {CHARACTERS.map((c) => {
            ...
            return (
              <button key={c.id}
                ...
                style={{ ...px(activeCh ? T.gold : T.outline), background: activeCh ? T.panelSolid : "rgba(20,22,52,0.9)", boxShadow: activeCh ? GLOW : "none", width: 74, height: 74, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, opacity: owned || wallet >= c.price ? 1 : 0.5 }}>
                <span style={{ fontSize: 26 }}>{owned ? c.emoji : "🔒"}</span>
        ...
        <div style={label}>{t("achTitle")}</div>
        <div style={{ display: "grid", gap: 8 }}>
          {achievements.map((a) => (
            <div key={a.id} className="hand" style={{ ...px(a.ok ? T.gold : T.outline), background: a.ok ? "rgba(60,50,20,0.6)" : "rgba(20,22,52,0.6)", color: a.ok ? T.gold : "#5a577e", padding: "9px 13px", fontSize: 18, display: "flex", gap: 10, alignItems: "center" }}>
        ...
        <div style={label}>{t("proTitle")} · 990 ₽</div>
        {settings.pro ? (
          <div className="pxfont" style={{ ...px(T.green), background: "rgba(30,60,40,0.8)", color: T.green, padding: "14px 14px", fontSize: 10, lineHeight: 1.7 }}>{t("proActive")}</div>
        ) : (
        ...
        <p className="pxfont" style={{ textAlign: "center", color: "#55537a", fontSize: 8, marginTop: 28, lineHeight: 1.8 }}>«ЧИТАЕМ ВМЕСТЕ» · v1.0</p>
      </div>
    </div>
  );
}
          ...
          {all.map((q) => (
            <div key={q.id} style={{ ...px(), background: T.paperMsg, color: T.ink, boxShadow: `4px 4px 0 rgba(0,0,0,0.35)`, padding: "14px 14px" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                ...
                <button className="pxfont" style={{ ...px(), background: "#3a3a52", color: T.muted, padding: "8px 12px", fontSize: 10, cursor: "pointer" }}
    ...
    <div className="view" style={{ maxWidth: 600, margin: "0 auto", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ padding: "22px 16px 40px" }}>
            ...
            return (
              <div key={c.id} style={{ display: "flex", alignItems: "stretch", gap: 8 }}>
                <button onClick={() => onOpen(c.id)}
                  style={{ flex: 1, minWidth: 0, textAlign: "left", background: T.paperMsg, color: T.ink, ...px(c.finished ? T.gold : T.outline), boxShadow: `4px 4px 0 rgba(0,0,0,0.35)`, padding: "12px 14px", cursor: "pointer" }}>
                  ...
                  <div className="hand" style={{ fontSize: 17, marginTop: 4, color: "#6b573a" }}>
                    {c.finished ? `🏁 ${t("finished")}` : `🔖 ${c.progress}`} · {comp.glyph} {comp.name[lang] || comp.name.ru} · {t("msgs")}: {c.messages.filter((m) => !m.meta && !m.hidden).length}
                    {c.goal ? ` · 🎯` : ""}
                  </div>
                </button>
                <button onClick={() => (confirming ? (onDelete(c.id), setConfirmId(null)) : setConfirmId(c.id))}
                  className="pxfont"
                  style={{ ...px(), background: confirming ? T.red : "#3a3a52", color: confirming ? "#fff" : T.muted, padding: "0 12px", fontSize: confirming ? 8 : 12, cursor: "pointer", flexShrink: 0 }}>
                  {confirming ? t("sure") : "✕"}
                </button>
              </div>
            );
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 14 }}>
          <button onClick={() => { setMoodOpen((v) => !v); setRec(null); setBlind(null); }} disabled={chats.length === 0} className="pxfont" style={{ ...px(moodOpen ? T.gold : T.outline), background: "#3a3a52", color: chats.length ? T.gold : "#55537a", padding: "13px 4px", fontSize: 9, cursor: "pointer", lineHeight: 1.5 }}>{t("recommend")}</button>
          <button onClick={getBlind} className="pxfont" style={{ ...px(), background: "#3a3a52", color: T.gold, padding: "13px 4px", fontSize: 9, cursor: "pointer", lineHeight: 1.5 }}>{t("blindLbl")}</button>
        </div>

        {moodOpen && (
          <div style={{ ...px(T.gold), background: "rgba(20,22,52,0.95)", marginTop: 12, padding: "12px 12px" }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {[["any", t("mAny")], ["sim", t("mSim")], ["new", t("mNew")], ["light", t("mLight")], ["deep", t("mDeep")], ["short", t("mShort")]].map(([id, label]) => (
                <button key={id} onClick={() => getRec(id)} className="pxfont" style={{ ...px(), background: "#3a3a52", color: T.muted, padding: "10px 11px", fontSize: 8.5, cursor: "pointer", lineHeight: 1.5 }}>{label}</button>
              ))}
            </div>
          </div>
        )}
    ...
    <div className="view" style={{ maxWidth: 600, margin: "0 auto", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ padding: "18px 16px 40px" }}>
          ...
          {COMPANIONS.map((c) => {
            ...
            return (
              <button key={c.id} onClick={() => !locked && setCompanion(c)}
                style={{ display: "flex", alignItems: "center", gap: 12, textAlign: "left", background: active ? T.panelSolid : "rgba(20, 22, 52, 0.9)", ...px(active ? T.gold : T.outline), boxShadow: active ? GLOW : "none", padding: "12px 13px", cursor: locked ? "default" : "pointer", opacity: locked ? 0.55 : 1 }}>
                <span style={{ width: 44, height: 44, flexShrink: 0, background: T.wood, ...px(), display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{locked ? "🔒" : c.glyph}</span>
                <span>
        ...
        <button disabled={!ready}
          onClick={() => onStart({ book: book.trim(), progress: progress.trim(), companionId: companion.id, bookInfo: bookInfo.trim(), goal: goal.trim() })}
      ...
      if (chat.messages.length === 0) {
        ...
        try {
          const hello = await askClaude(
            [{ role: "user", content: "Поздоровайся со мной как собеседник по этой книге: коротко, в своём характере. Если ты не знаешь эту книгу — честно скажи это прямо сейчас. Задай один вопрос. Без спойлеров." }],
    ...
    <div className="view" style={{ maxWidth: 640, margin: "0 auto", minHeight: "100vh", display: "flex", flexDirection: "column", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 5, background: T.nightDeep, borderBottom: `4px solid ${T.outline}` }}>
        ...
        <div style={{ padding: "10px 14px 12px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
            ...
            <button onClick={onBack} className="pxfont" style={{ ...px(), background: "#3a3a52", color: T.muted, padding: "9px 10px", fontSize: 8.5, cursor: "pointer", flexShrink: 0 }}>{t("shelf")}</button>
          </div>

          <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
            {!finished && (
              <button onClick={() => setEditingProgress((v) => !v)} className="hand"
                style={{ display: "inline-flex", alignItems: "center", gap: 6, background: T.wood, color: "#fff2d9", ...px(), borderLeft: `8px solid ${T.gold}`, padding: "6px 12px", fontSize: 17, cursor: "pointer" }}>
                🔖 {t("here")}: {progress} <span style={{ opacity: 0.7, fontSize: 15 }}>· {t("edit")}</span>
              </button>
            )}
            <button onClick={() => setToolsOpen((v) => !v)} className="pxfont" style={{ ...px(toolsOpen ? T.gold : T.outline), background: "#3a3a52", color: T.gold, padding: "8px 11px", fontSize: 9, cursor: "pointer" }}>{t("tools")}</button>
          </div>
          ...
          {pendingEmo && (
            <div style={{ display: "flex", gap: 8, marginTop: 10, alignItems: "center", flexWrap: "wrap" }}>
              ...
              {EMOJIS.map((e) => (
                <button key={e} onClick={() => pickEmo(e)} style={{ ...px(), background: "#3a3a52", fontSize: 20, padding: "5px 9px", cursor: "pointer", lineHeight: 1 }}>{e}</button>
              ))}
        ...
        {messages.filter((m) => !m.hidden).map((m) =>
          m.meta === "divider" ? (
            ...
            <div key={m.id} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "92%", display: "flex", flexDirection: m.role === "user" ? "row-reverse" : "row", alignItems: "flex-start", gap: 8 }}>
              {m.role === "assistant" && (
                <span aria-hidden style={{ width: 32, height: 32, flexShrink: 0, marginTop: 2, background: T.wood, border: `3px solid ${T.outline}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, boxShadow: `2px 2px 0 rgba(0,0,0,0.35)` }}>{companion.glyph}</span>
              )}
      ...
      <div style={{ position: "sticky", bottom: 0, background: T.nightDeep, borderTop: `4px solid ${T.outline}`, padding: "12px 14px calc(14px + env(safe-area-inset-bottom, 0px))" }}>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          {SR && (
            <button onClick={toggleVoice} className="pxfont" style={{ ...px(listening ? T.red : T.outline), background: listening ? T.red : "#3a3a52", color: "#fff", padding: "0 12px", fontSize: 16, cursor: "pointer", flexShrink: 0 }}>🎤</button>
          )}
          ...
          <div style={{ display: "grid", gap: 6 }}>
            {emo.map((e, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ flex: 1, height: 12, ...px(), background: "#1c1a3a" }}>
      ...
      {panel === "chatSettings" && (
        <Panel title="⚙️" onClose={() => setPanel(null)} t={t}>
          <button onClick={() => setSlow(!slow)} className="hand" style={{ display: "flex", justifyContent: "space-between", width: "100%", background: "rgba(20,22,52,0.9)", ...px(slow ? T.gold : T.outline), color: slow ? T.gold : T.muted, padding: "11px 14px", fontSize: 18, cursor: "pointer", marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
            <button onClick={() => setDiscussLang(null)} className="pxfont" style={{ ...px(!discussLang ? T.gold : T.outline), background: "#3a3a52", color: !discussLang ? T.gold : T.muted, padding: "8px 10px", fontSize: 9, cursor: "pointer" }}>{t("offLbl").toUpperCase()}</button>
            {LANGS.map((l) => (
              <button key={l.id} onClick={() => setDiscussLang(l.id)} style={{ ...px(discussLang === l.id ? T.gold : T.outline), background: "#3a3a52", boxShadow: discussLang === l.id ? GLOW : "none", fontSize: 20, padding: "5px 9px", cursor: "pointer", lineHeight: 1 }}>{l.flag}</button>
            ))}
          </div>
...
export default function App() {
  ...
  useEffect(() => {
    loadState().then(({ chats, settings }) => { setChats(chats); setSettings(settings); });
  }, []);
  ...
  function persistSettings(next) { setSettings(next); saveSettings(next); }
  ...
  function createChat(data) {
    const chat = { id: Date.now(), ...data, messages: [], quotes: [], notes: [], predictions: [], emo: [], charMap: "", slow: false, discussLang: null, polyLevel: "", finished: false, lastSeen: Date.now(), createdAt: Date.now() };
  ...
  return (
    <div style={{ background: `linear-gradient(${T.night}, ${T.nightDeep})`, minHeight: "100vh", color: T.white }}>
      {FONTS}
      <Stars />
      <div aria-live="polite" style={{ position: "fixed", left: 0, right: 0, bottom: 96, zIndex: 70, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, pointerEvents: "none" }}>
        {toasts.map((x) => (
      ...
      {chats === null ? (
        ...
        <Menu t={t} settings={settings} chats={chats} onChange={persistSettings} onBack={() => setView("library")} onImport={importAll} />
      ) : view === "quotes" ? (
        ...
        <Library t={t} lang={settings.lang} chats={chats} settings={settings}
          ...
          onNew={() => { if (chats.length < limit) { setView("setup"); } else if (!settings.pro) { setView("menu"); } }}
          onDelete={deleteChat} onMenu={() => setView("menu")} onQuotes={() => setView("quotes")} onProfile={() => setView("profile")}
